...
app.post('/keywords', function (req, res) {
	...
	alchemyapi.keywords(flavor, data, options, function (response) {
		if (response.status === 'ERROR') {
			console.log(response.statusInfo);
			res.send(500, response.status);
		} else {
			res.json(response);
		}
	});
...
app.post('/concepts', function (req, res) {
	var flavor = req.body.flavor,
		data = req.body.data,
		options = req.body.options;
	alchemyapi.concepts(flavor, data, options, function (response) {
		if (response.status === 'ERROR') {
			console.log(response.statusInfo);
			res.send(response);
		} else {
...
app.post('/taxonomy', function (req, res) {
	var flavor = req.body.flavor,
		data = req.body.data,
		options = req.body.options;
	console.log(data);
	alchemyapi.taxonomy(flavor, data, options, function (response) {
		if (response.status === 'ERROR') {
			console.log(response.statusInfo);
			res.send(500, response.status);
		} else {
